...
for cat in categorias:
    obj, was_created = Category.objects.get_or_create(name=cat['name'], defaults=cat)
    if was_created:
        created += 1
        print('Creada:', cat['name'])
    else:
        print('Ya existe:', cat['name'])

print('Total en BD:', Category.objects.count(), '| Nuevas:', created)
